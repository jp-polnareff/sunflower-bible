package com.leetcode.editor.leetcode.editor.cn;  //请你来实现一个 myAtoi(string s) 函数，使其能将字符串转换成一个 32 位有符号整数（类似 C/C++ 中的 atoi 函数）。
//
...
public class StringToIntegerAtoi {
    ...
    class Solution {
        public int myAtoi(String s) {
            ...
            if (index < s.length() && (chars[index] == '-' || chars[index] == '+')) {
                sign = chars[index] == '-' ? -1 : 1;
                index++;
            }
            while (index < s.length() && Character.isDigit(chars[index])) {
                int num = Character.getNumericValue(chars[index]);
